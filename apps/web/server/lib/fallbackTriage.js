import { computeReadiness, weightedAverageReadiness } from './readinessFormula.js'
...
export function createFallbackTriage({ intent, taxon, preview, reason }) {
  ...
  const average = weightedAverageReadiness(readiness, intent)
  ...
  return {
    support: fallbackSupport({ total, coordinateShare, dateShare, average, unsupportedClaims }),
    risks,
    readiness,
    recommendedFilters: fallbackFilters({ intent, total, coordinateShare, dateShare, preview }),
    unsupportedClaims,
    nextSteps: fallbackNextSteps({ total, average }),
  }
}
...
function fallbackSupport({ total, coordinateShare, dateShare, average, unsupportedClaims }) {
  if (total <= 0) {
    return {
  ...
  return {
    headline: fallbackHeadline(average),
    stronglySupported:
      average >= 75 && unsupportedClaims.length === 0
...
function fallbackHeadline(average) {
  if (average >= 70) return 'GBIF looks usable for a cautious, filter-driven workflow.'
  if (average >= 40) return 'GBIF can support exploratory planning, but key limitations need review.'
...
function fallbackNextSteps({ total, average }) {
  if (total <= 0) {
    return [
